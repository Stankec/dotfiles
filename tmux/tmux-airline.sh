...
if [ "$WIDTH" -gt "$MEDIUM" ]; then
  ...
  MEMORY="#[fg=colour236,bg=${date_colour:-default},nobold,noitalics,nounderscore]$SEP#[fg=colour247,bg=colour236,nobold,noitalics,nounderscore] $MEMORY #(~/.tmux/used_memory.sh)/#(~/.tmux/total_memory.sh)M"
...
if [ "$WIDTH" -ge "$SMALL" ]; then
  ...
  DATE="#[fg=colour236,bg=${date_colour:-default},nobold,noitalics,nounderscore]$SEP#[fg=colour247,bg=colour236,nobold,noitalics,nounderscore] $CALENDAR $(date +'%Y/%m/%d')"
